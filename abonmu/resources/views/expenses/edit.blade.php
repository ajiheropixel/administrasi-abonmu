...
<div class="max-w-2xl">
    <div class="bg-white rounded-lg shadow-sm border border-gray-200">
        <div class="px-6 py-4 border-b border-gray-200">
            <h3 class="text-lg font-semibold text-gray-800">Form Edit Pengeluaran</h3>
        </div>

        <form action="{{ route('expenses.update', $expense) }}" method="POST" class="p-6 space-y-4">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <select name="production_id"
                    ...
                    @foreach($productions as $production)
                        <option value="{{ $production->id }}" {{ old('production_id', $expense->production_id) == $production->id ? 'selected' : '' }}>
            ...
            <div>
                ...
                <textarea name="description" rows="3"
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">{{ old('description', $expense->description) }}</textarea>
            </div>
