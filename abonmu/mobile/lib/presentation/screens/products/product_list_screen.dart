...
class _ProductListScreenState extends State<ProductListScreen> {
  ...
  void _openForm(BuildContext context, {ProductModel? product}) {
    Navigator.push(
      ...
      MaterialPageRoute(
        ...
        (_) => context.read<ProductProvider>().loadProducts(refresh: true));
  }
  ...
  Future<void> _delete(BuildContext context, ProductModel product) async {
    final ok = await showConfirmDialog(
      ...
      title: 'Hapus Produk',
      message:
          'Hapus "${product.name}"? Tindakan ini tidak dapat dibatalkan.',
    );
    ...
    if (success) {
      ...
      showErrorSnackbar(context,
          context.read<ProductProvider>().error ?? 'Gagal menghapus');
    }
  }
}
